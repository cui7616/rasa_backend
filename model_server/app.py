...
def save_file():
    ...
    with open(models_dir+'/models/'+file_name, 'ab+') as f:
        f.write(data)
    return "已接收保存"
...
def download_file(path):
    return send_from_directory(dirname(path), basename(path), as_attachment=True, attachment_filename=basename(path),add_etags=True)
